from verticalWall import VerticalWall
from horizontalWall import HorizontalWall


class WallAdministration:
    horizontal_walls = []
    vertical_walls = []
    north_walls = []
    south_walls = []
    east_walls = []
    west_walls = []
    walls = None

    def __init__(self, walls):
        self.walls = walls

    def split_horizontal_and_vertical_walls(self):
        for i in range(len(self.walls)):
            if self.walls[i].get_coordinate1()[0] == self.walls[i].get_coordinate2()[0]:
                self.vertical_walls.append(VerticalWall(self.walls[i].get_coordinate1(
                ), self.walls[i].get_coordinate2(), self.walls[i].get_range()))
            else:
                self.horizontal_walls.append(HorizontalWall(self.walls[i].get_coordinate1(
                ), self.walls[i].get_coordinate2(), self.walls[i].get_range()))

    def get_all_walls_between_range(self, orientation, length):

        if orientation == "horizontal":
            all_walls_between_range = []
            for i in range(len(self.horizontal_walls)):
                if self.horizontal_walls[i].get_range()[0] < length[1] and self.horizontal_walls[i].get_range()[1] > length[0]:
                    all_walls_between_range.append(self.horizontal_walls[i])
        else:
            all_walls_between_range = []
            for i in range(len(self.vertical_walls)):
                if self.vertical_walls[i].get_range()[0] < length[1] and self.vertical_walls[i].get_range()[1] > length[0]:
                    all_walls_between_range.append(self.vertical_walls[i])

        return all_walls_between_range

    def bring_all_ranges_in_right_order(self, orientation):

        if orientation == "horizontal":

            for i in range(len(self.horizontal_walls)):
                if self.horizontal_walls[i].get_range()[0] > self.horizontal_walls[i].get_range()[1]:
                    self.horizontal_walls[i].set_range([self.horizontal_walls[i].get_range()[
                        1], self.horizontal_walls[i].get_range()[0]])
        else:
            for i in range(len(self.vertical_walls)):
                if self.vertical_walls[i].get_range()[0] > self.vertical_walls[i].get_range()[1]:
                    self.vertical_walls[i].set_range([self.vertical_walls[i].get_range()[
                        1], self.vertical_walls[i].get_range()[0]])

    def set_range(self):

        for wall in self.vertical_walls:

            length = [wall.get_coordinate1()[1],
                      wall.get_coordinate2()[1]]
            wall.set_range(length)


     def get_north_walls(self):
        return self.north_walls

    def get_south_walls(self):
        return self.north_walls

    def get_walls(self):
        return self.horizontal_walls

    def set_range(self):

        for wall in self.horizontal_walls:

            length = [wall.get_coordinate1()[0],
                      wall.get_coordinate2()[0]]
            wall.set_range(length)

    def find_all_definitely_north_walls(self):

        self.bring_all_ranges_in_right_order(self.horizontal_walls)

        for i in range(len(self.horizontal_walls)):
            north_wall = self.horizontal_walls[i]
            horizontal_walls_in_same_range = self.get_all_walls_between_range(self.horizontal_walls,
                                                                              self.horizontal_walls[i].get_range())
            for j in range(len(horizontal_walls_in_same_range)):
                if horizontal_walls_in_same_range[j].get_coordinate1()[1] > north_wall.get_coordinate1()[1]:

                    north_wall = horizontal_walls_in_same_range[j]
            if north_wall not in self.north_walls:
                self.north_walls.append(north_wall)

    def find_all_definitely_south_walls(self):

        self.bring_all_ranges_in_right_order(self.horizontal_walls)

        for i in range(len(self.horizontal_walls)):
            south_wall = self.horizontal_walls[i]
            horizontal_walls_in_same_range = self.get_all_walls_between_range(self.horizontal_walls,
                                                                              self.horizontal_walls[i].get_range())
            for j in range(len(horizontal_walls_in_same_range)):
                if horizontal_walls_in_same_range[j].get_coordinate1()[1] < south_wall.get_coordinate1()[1]:

                    south_wall = horizontal_walls_in_same_range[j]
            if south_wall not in self.south_walls:
                self.south_walls.append(south_wall)

    def find_adjacent_walls(self, wall_list, list_of_walls):
        possible_adjacents = []
        adjacent_north_and_south_wall = {}

        for i in range(len(wall_list)):
            self.check_north_wall(wall_list[i])
            print(wall_list[i].get_range())
            print("+++++++++++++++++++++++++++++")
            possible_adjacents = self.get_all_walls_between_range(
                list_of_walls, wall_list[i].get_range())
            if not wall_list[i] == possible_adjacents[0]:
                adjacent = possible_adjacents[0]

            for j in range(len(possible_adjacents)):
                if (wall_list[i].get_coordinate1()[1] - possible_adjacents[j].get_coordinate1()[1]) < (wall_list[i].get_coordinate1()[1] - adjacent.get_coordinate1()[1]) and not wall_list[i] == possible_adjacents[j]:
                    print("if")
                    adjacent = possible_adjacents[j]
                    print(adjacent.get_range())
            adjacent_north_and_south_wall[wall_list[i]] = adjacent
        return adjacent_north_and_south_wall

    def check_north_wall(self, north_wall):
        height1 = north_wall.get_coordinate1()[1]
        height2 = north_wall.get_coordinate2()[1]
        print("function call")
        for i in range(len(self.vertical_walls)):
            print(".....................................................")
            print(self.vertical_walls[i].get_range())
            print(".....................................................")
            if self.vertical_walls[i].get_range()[0] < height1 < self.vertical_walls[i].get_range()[1]:
                print("if1")
                print(north_wall.get_range()[1])
                north_wall.set_range([self.vertical_walls[i].get_coordinate1()[
                                     0], north_wall.get_range()[1]])
                print(north_wall.get_range())

            if self.vertical_walls[i].get_range()[0] < height2 < self.vertical_walls[i].get_range()[1]:
                print("if2")
                north_wall.set_range(
                    [north_wall.get_range()[0], self.vertical_walls[i].get_coordinate2()[0]])
            print(north_wall.get_range())
            print("jklöööööööööööööööööööööööööööööööööööööööö")
